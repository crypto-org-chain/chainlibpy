...
from chainlibpy.transaction import gen_multi_tx, gen_packed_send_msg
...
class ChainMainClient(object):
    ...
    def __init__(self, data_root=Path("./data"), config_file="config.yaml", chain_id="chain_id_test"):
...
def prepare():
    ...
    # start the chain-maind
    # client.start()
    # 1. create multi wallet
    cluster = client.cluster
    try:
        cluster.make_multisig(MULTI_WALLET, "msigner0", "msigner1")
    except:
        pass
    multi_addr = cluster.address(MULTI_WALLET)
...
def main():
    ...
    # multi_address = prepare()
    # print(multi_address)
    multi_address = "cro12s2gcgzxg7ugucmeu575rmtszrqf2y5xtaug6x"
    ...
    res = grpc_client.query_account_balance(multi_address)
    print(f"get multi address balance: {res.balance.amount}")
    account_info = grpc_client.query_account(multi_address)
    ...
    msg = gen_packed_send_msg(multi_address, to_address, amount)
    tx = gen_multi_tx(
        [msg],
        multi_pubkey,
        sig_batch,
    ...
    response = grpc_client.broadcast_transaction(tx.SerializeToString())
